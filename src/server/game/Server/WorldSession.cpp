...
{
    char const* format = GetCoreString(string_id);
    if (format)
    {
        va_list ap;
        char szStr [1024];
...
const char * WorldSession::GetCoreString(int32 entry) const
{
    return sObjectMgr->GetCoreString(entry, GetSessionDbLocaleIndex());
}
