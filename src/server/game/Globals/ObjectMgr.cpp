...
void ObjectMgr::CheckCreatureTemplate(CreatureInfo const* cInfo)
{
    ...
    if (cInfo->expansion > (MAX_CREATURE_BASE_EXP - 1))
    {
...
bool ObjectMgr::LoadCoreStrings(char const* table, int32 min_value, int32 max_value)
{
    ...
    for (CoreStringLocaleMap::iterator itr = mCoreStringLocaleMap.begin(); itr != mCoreStringLocaleMap.end();)
    {
        if (itr->first >= start_value && itr->first < end_value)
            mCoreStringLocaleMap.erase(itr++);
        else
            ++itr;
    }
    ...
    {
        sLog->outString();
        if (min_value == MIN_CORE_STRING_ID)              // error only in case internal strings
            sLog->outErrorDb(">> Loaded 0 Core Strings. DB table `%s` is empty. Cannot continue.", table);
        else
    ...
    do
    {
        ...
        CoreStringLocale& data = mCoreStringLocaleMap[entry];
    ...
    sLog->outString();
    if (min_value == MIN_CORE_STRING_ID)
        sLog->outString(">> Loaded %u Core Strings from table %s", count, table);
    else
...
const char *ObjectMgr::GetCoreString(int32 entry, LocaleConstant locale_idx) const
{
    if (CoreStringLocale const *msl = GetCoreStringLocale(entry))
    {
    ...
    if (entry > 0)
        sLog->outErrorDb("Entry %i not found in `core_string` table.", entry);
    else
    ...
    for (ScriptMapMap::const_iterator itrMM = scripts->begin(); itrMM != scripts->end(); ++itrMM)
    {
        for (ScriptMap::const_iterator itrM = itrMM->second.begin(); itrM != itrMM->second.end(); ++itrM)
        {
            switch(itrM->second.command)
            {
                case SCRIPT_COMMAND_TALK:
                {
                    if (!GetCoreStringLocale (itrM->second.Talk.TextID))
...
{
    LoadCoreStrings("db_script_string", MIN_DB_SCRIPT_STRING_ID, MAX_DB_SCRIPT_STRING_ID);

    std::set<int32> ids;

    for (int32 i = MIN_DB_SCRIPT_STRING_ID; i < MAX_DB_SCRIPT_STRING_ID; ++i)
        if (GetCoreStringLocale(i))
            ids.insert(i);
...
bool LoadCoreStrings(char const* table, int32 start_value, int32 end_value)
{
    ...
    return sObjectMgr->LoadCoreStrings(table, start_value, end_value);
}
...
CreatureBaseStats const* ObjectMgr::GetCreatureBaseStats(uint8 level, uint8 unitClass)
{
    ...
    struct DefaultCreatureBaseStats : public CreatureBaseStats
    {
        ...
        {
            BaseArmor = 1;
            for (uint8 j = 0; j < MAX_CREATURE_BASE_EXP; ++j)
                BaseHealth[j] = 1;
            BaseMana = 0;
        }
    };
...
void ObjectMgr::LoadCreatureClassLevelStats()
{
    ...
    do
    {
        ...
        for (uint8 i = 0; i < MAX_CREATURE_BASE_EXP; ++i)
