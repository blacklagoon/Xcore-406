...
const char *ChatHandler::GetCoreString(int32 entry) const
{
    return m_session->GetCoreString(entry);
}
...
{
    SendSysMessage(GetCoreString(entry));
}

void ChatHandler::PSendSysMessage(int32 entry, ...)
{
    const char *format = GetCoreString(entry);
    va_list ap;
    char str [2048];
    va_start(ap, entry);
...
{
    return GetCoreString(LANG_CONSOLE_COMMAND);
}
