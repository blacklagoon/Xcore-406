...
bool ChatHandler::HandleAnnounceCommand(const char* args)
{
    ...
    char buff[2048];
    sprintf(buff, GetCoreString(LANG_SYSTEMMESSAGE), args);
...
bool ChatHandler::HandleNotifyCommand(const char* args)
{
    ...
    std::string str = GetCoreString(LANG_GLOBAL_NOTIFY);
    str += args;
...
bool ChatHandler::HandleGMNotifyCommand(const char* args)
{
    ...
    std::string str = GetCoreString(LANG_GM_NOTIFY);
    str += args;
...
bool ChatHandler::HandleGMVisibleCommand(const char* args)
{
    if (!*args)
    {
        PSendSysMessage(LANG_YOU_ARE, m_session->GetPlayer()->isGMVisible() ?  GetCoreString(LANG_VISIBLE) : GetCoreString(LANG_INVISIBLE));
        return true;
    }
...
bool ChatHandler::HandleSummonCommand(const char* args)
{
    ...
    else
    {
        ...
        PSendSysMessage(LANG_SUMMONING, nameLink.c_str(), GetCoreString(LANG_OFFLINE));
...
bool ChatHandler::HandleModifyEnergyCommand(const char* args)
{
    ...
    sLog->outDetail(GetCoreString(LANG_CURRENT_ENERGY), chr->GetMaxPower(POWER_ENERGY));
...
bool ChatHandler::HandleModifyMoneyCommand(const char* args)
{
    ...
    if (addmoney < 0)
    {
        ...
        sLog->outDetail(GetCoreString(LANG_CURRENT_MONEY), moneyuser, addmoney, newmoney);
        if (newmoney <= 0)
        {
    ...
    sLog->outDetail(GetCoreString(LANG_NEW_MONEY), moneyuser, addmoney, chr->GetMoney());
...
bool ChatHandler::HandleWhispersCommand(const char* args)
{
    if (!*args)
    {
        PSendSysMessage(LANG_COMMAND_WHISPERACCEPTING, m_session->GetPlayer()->isAcceptWhispers() ?  GetCoreString(LANG_ON) : GetCoreString(LANG_OFF));
        return true;
    }
