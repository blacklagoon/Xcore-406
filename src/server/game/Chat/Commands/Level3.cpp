...
bool ChatHandler::HandleAddItemCommand(const char *args)
{
    ...
    sLog->outDetail(GetCoreString(LANG_ADDITEM), itemId, count);
...
bool ChatHandler::HandleAddItemSetCommand(const char *args)
{
    ...
    sLog->outDetail(GetCoreString(LANG_ADDITEMSET), itemsetId);
    ...
    for (uint32 id = 0; id < sSkillLineStore.GetNumRows(); id++)
    {
        ...
        if (skillInfo)
        {
            ...
            if (!name.empty())
            {
                ...
                {
                    knownStr = GetCoreString(LANG_KNOWN);
                    uint32 curValue = target->GetPureSkillValue(id);
                    uint32 maxValue  = target->GetPureMaxSkillValue(id);
                    uint32 permValue = target->GetSkillPermBonusValue(id);
                    uint32 tempValue = target->GetSkillTempBonusValue(id);

                    char const* valFormat = GetCoreString(LANG_SKILL_VALUES);
    ...
    for (uint32 id = 0; id < sSpellStore.GetNumRows(); id++)
    {
        ...
        if (spellInfo)
        {
            ...
            if (loc < TOTAL_LOCALES)
            {
                ...
                if (rank)
                    ss << GetCoreString(LANG_SPELL_RANK) << rank;
                ...
                if (talent)
                    ss << GetCoreString(LANG_TALENT);
                if (passive)
                    ss << GetCoreString(LANG_PASSIVE);
                if (learn)
                    ss << GetCoreString(LANG_LEARN);
                if (known)
                    ss << GetCoreString(LANG_KNOWN);
                if (active)
                    ss << GetCoreString(LANG_ACTIVE);
                        ...
                        if (target)
                        {
                            ...
                            if (status == QUEST_STATUS_COMPLETE)
                            {
                                if (target->GetQuestRewardStatus(qinfo->GetQuestId()))
                                    statusStr = GetCoreString(LANG_COMMAND_QUEST_REWARDED);
                                else
                                    statusStr = GetCoreString(LANG_COMMAND_QUEST_COMPLETE);
                            }
                            else if (status == QUEST_STATUS_INCOMPLETE)
                                statusStr = GetCoreString(LANG_COMMAND_QUEST_ACTIVE);
                        }
        ...
        if (factionEntry)
        {
            ...
            if (!name.empty())
            {
                ...
                if (repState)                               // and then target != NULL also
                {
                    ...
                    std::string rankName = GetCoreString(index);
                    ...
                    ss << GetCoreString(LANG_FACTION_NOREPUTATION);
        ...
        if (MapInfo)
        {
            ...
            if (loc < TOTAL_LOCALES)
            {
                ...
                if (MapInfo->IsContinent())
                    ss << GetCoreString(LANG_CONTINENT);

                switch(MapInfo->map_type)
                {
                    case MAP_INSTANCE:      ss << GetCoreString(LANG_INSTANCE);      break;
                    case MAP_BATTLEGROUND:  ss << GetCoreString(LANG_BATTLEGROUND);  break;
                    case MAP_ARENA:         ss << GetCoreString(LANG_ARENA);         break;
                }

                if (MapInfo->IsRaid())
                    ss << GetCoreString(LANG_RAID);

                if (MapInfo->SupportsHeroicMode())
                    ss << GetCoreString(LANG_HEROIC);
                ...
                if (MapInfo->IsMountAllowed())
                    ss << GetCoreString(LANG_MOUNTABLE);
...
bool ChatHandler::HandleNearGraveCommand(const char *args)
{
    ...
    if (graveyard)
    {
        ...
        std::string team_name = GetCoreString(LANG_COMMAND_GRAVEYARD_NOTEAM);

        if (g_team == 0)
            team_name = GetCoreString(LANG_COMMAND_GRAVEYARD_ANY);
        else if (g_team == HORDE)
            team_name = GetCoreString(LANG_COMMAND_GRAVEYARD_HORDE);
        else if (g_team == ALLIANCE)
            team_name = GetCoreString(LANG_COMMAND_GRAVEYARD_ALLIANCE);
...
bool ChatHandler::HandleListAurasCommand (const char * /*args*/)
{
    ...
    char const* talentStr = GetCoreString(LANG_TALENT);
    char const* passiveStr = GetCoreString(LANG_PASSIVE);
...
bool ChatHandler::HandleBanInfoCharacterCommand(const char *args)
{
    ...
    do
    {
        ...
        std::string bantime = permanent ? GetCoreString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[1].GetUInt32(), true);
        PSendSysMessage(LANG_BANINFO_HISTORYENTRY,
            fields[0].GetCString(), bantime.c_str(), active ? GetCoreString(LANG_BANINFO_YES) : GetCoreString(LANG_BANINFO_NO), fields[4].GetCString(), fields[5].GetCString());
    }
    while (result->NextRow());
...
bool ChatHandler::HandleBanInfoHelper(uint32 accountid, char const* accountname)
{
    ...
    do
    {
        ...
        std::string bantime = permanent ? GetCoreString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[1].GetUInt64(), true);
        PSendSysMessage(LANG_BANINFO_HISTORYENTRY,
            fields[0].GetCString(), bantime.c_str(), active ? GetCoreString(LANG_BANINFO_YES) : GetCoreString(LANG_BANINFO_NO), fields[4].GetCString(), fields[5].GetCString());
    } while (result->NextRow());
...
bool ChatHandler::HandleBanInfoIPCommand(const char *args)
{
    ...
    PSendSysMessage(LANG_BANINFO_IPENTRY,
        fields[0].GetCString(), fields[1].GetCString(), permanent ? GetCoreString(LANG_BANINFO_NEVER) : fields[2].GetCString(),
        permanent ? GetCoreString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[3].GetUInt64(), true).c_str(), fields[4].GetCString(), fields[5].GetCString());
